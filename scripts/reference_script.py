...
def run():
    ...
    test_opts.ref_images_paths_file='experiment/ref.txt'
    test_opts.target_age='50,60,70,80'
    test_opts.latent_mask='8,9'
    test_opts.mix_alpha=None

    out_path_results = os.path.join(test_opts.exp_dir, 'inference-endpoint')
    os.makedirs(out_path_results, exist_ok=True)

    # update test options with options used during training
    ckpt = torch.load(test_opts.checkpoint_path, map_location='gpu')
    opts = ckpt['opts']
